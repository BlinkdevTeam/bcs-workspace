...
// ── Mock data — replace with API calls in Sprint 3 ────────────────────────────
// parent_id: null = top-level task, parent_id: N = subtask of task N
const MOCK_TASKS = [
  { id: 1,  name: "Set up Redux store",                priority: "high",   status: "progress", project: "BCS Workspace", due: "2026-03-13", person: "KM", done: false, parent_id: null },
  { id: 2,  name: "Configure Axios interceptors",      priority: "high",   status: "progress", project: "BCS Workspace", due: "2026-03-13", person: "KM", done: false, parent_id: null },
  { id: 3,  name: "Build ProtectedRoute component",    priority: "medium", status: "todo",     project: "BCS Workspace", due: "2026-03-14", person: "KM", done: false, parent_id: null },
  { id: 4,  name: "Health check endpoint + CORS",      priority: "medium", status: "review",   project: "BCS Workspace", due: "2026-03-13", person: "JD", done: false, parent_id: null },
  { id: 5,  name: "Create tasks-backend boilerplate",  priority: "low",    status: "done",     project: "BCS Workspace", due: "2026-03-10", person: "KM", done: true,  parent_id: null },
  // Subtasks of task 1
  { id: 11, name: "Install redux toolkit",             priority: "high",   status: "done",     project: "BCS Workspace", due: "2026-03-13", person: "KM", done: true,  parent_id: 1 },
  { id: 12, name: "Create authSlice",                  priority: "medium", status: "progress", project: "BCS Workspace", due: "2026-03-13", person: "KM", done: false, parent_id: 1 },
  { id: 13, name: "Create uiSlice",                    priority: "low",    status: "todo",     project: "BCS Workspace", due: "2026-03-14", person: "KM", done: false, parent_id: 1 },
  // Subtasks of task 2
  { id: 14, name: "Set baseURL and withCredentials",   priority: "high",   status: "done",     project: "BCS Workspace", due: "2026-03-13", person: "KM", done: true,  parent_id: 2 },
  { id: 15, name: "Add request interceptor",           priority: "high",   status: "progress", project: "BCS Workspace", due: "2026-03-13", person: "KM", done: false, parent_id: 2 },
  { id: 16, name: "Add silent refresh on 401",         priority: "high",   status: "todo",     project: "BCS Workspace", due: "2026-03-13", person: "JD", done: false, parent_id: 2 },

  { id: 6,  name: "Design HRIS employee profile page", priority: "high",   status: "todo",     project: "HRIS Redesign", due: "2026-03-10", person: "AL", done: false, parent_id: null },
  { id: 7,  name: "Build department management UI",    priority: "medium", status: "progress", project: "HRIS Redesign", due: "2026-03-14", person: "JD", done: false, parent_id: null },
  { id: 8,  name: "Set up React Router for HRIS",      priority: "low",    status: "done",     project: "HRIS Redesign", due: "2026-03-09", person: "AL", done: true,  parent_id: null },
  // Subtasks of task 6
  { id: 17, name: "Design personal info tab",          priority: "high",   status: "todo",     project: "HRIS Redesign", due: "2026-03-10", person: "AL", done: false, parent_id: 6 },
  { id: 18, name: "Design job info tab",               priority: "medium", status: "todo",     project: "HRIS Redesign", due: "2026-03-10", person: "AL", done: false, parent_id: 6 },

  { id: 9,  name: "Design onboarding screens",         priority: "high",   status: "todo",     project: "Mobile App",    due: "2026-03-11", person: "KM", done: false, parent_id: null },
  { id: 10, name: "API integration for mobile auth",   priority: "medium", status: "progress", project: "Mobile App",    due: "2026-03-13", person: "JD", done: false, parent_id: null },
];
...
function dateColor(dateStr, done) {
  if (done) return "#444";
  const today = new Date(); today.setHours(0, 0, 0, 0);
  const d     = new Date(dateStr);
...
export default function MyTasks() {
  ...
  // ── Drag to reorder (parent tasks only) ──────────────────────────────────────
  ...
  const clearFilters = () => {
    setSearch("");
    setFilters({ priority: new Set(), project: new Set(), person: new Set(), date: new Set() });
  };
  ...
  // Only filter parent tasks — subtasks always follow their parent
  ...
  const filtered    = parentTasks.filter((t) => {
    ...
    if (filters.person.size   && !filters.person.has(t.person))     return false;
    ...
    <div style={s.page} onClick={() => setOpenMenu(null)}>
      ...
      <div style={s.tableWrapper}>
        <table style={s.table}>
          <thead>
            <tr>
              <th style={{ ...s.th, paddingLeft: 24, width: "38%" }}>Task</th>
              <th style={{ ...s.th, width: 140 }}>Status</th>
              <th style={{ ...s.th, width: 120 }}>Priority</th>
              <th style={{ ...s.th, width: 120 }}>Person</th>
...
// ── ProjectGroup ──────────────────────────────────────────────────────────────
function ProjectGroup({ project, tasks, allTasks, color, collapsed, onToggleCollapse, onUpdateTask, onAddSubtask, expandedTasks, onToggleExpand, dragState, onDragStart, onDragOver, onDragEnd }) {
  return (
    <>
      <tr>
          ...
          {tasks.map((task) => {
            ...
            return (
              <TaskRow
                ...
                isDragOver={dragState.over === task.id}
...
// ── TaskRow ───────────────────────────────────────────────────────────────────
function TaskRow({ task, subtasks = [], isExpanded, onToggleExpand, onUpdate, onAddSubtask, onUpdateSubtask, isDragging, isDragOver, onDragStart, onDragOver, onDragEnd, isSubtask }) {
  ...
  // sync nameVal when task.name changes externally
  ...
  const rowStyle = {
    ...
    background: isDragOver ? "rgba(59,130,246,0.04)" : isSubtask ? "#191919" : "#141414",
  };
      ...
      >
        ...
        <td style={{ paddingLeft: isSubtask ? 56 : 24, background: "transparent" }}>
          <div style={s.taskNameCell}>
            {/* Drag handle — parent only */}
            {!isSubtask && (
              <div style={s.dragHandle}>
                <svg width="10" height="14" viewBox="0 0 10 14" fill="none">
                  <circle cx="3" cy="2.5"  r="1" fill="#444" />
                  <circle cx="7" cy="2.5"  r="1" fill="#444" />
                  <circle cx="3" cy="7"    r="1" fill="#444" />
                  <circle cx="7" cy="7"    r="1" fill="#444" />
                  <circle cx="3" cy="11.5" r="1" fill="#444" />
                  <circle cx="7" cy="11.5" r="1" fill="#444" />
                </svg>
              </div>
            )}
            ...
            {/* Expand toggle — only on parent tasks that have subtasks */}
            {!isSubtask && (
              <div
                style={{ ...s.expandBtn, opacity: subtasks.length > 0 ? 1 : 0, pointerEvents: subtasks.length > 0 ? "auto" : "none" }}
            ...
            >
              {task.done && (
                <svg width="8" height="5" viewBox="0 0 8 5" fill="none">
                  <path d="M1 2.5L3 4.5L7 0.5" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              )}
            </div>
            ...
            {editingName ? (
              ...
              <span
                style={{ ...s.taskName, ...(task.done ? s.taskNameDone : {}), ...(isSubtask ? s.subtaskName : {}) }}
        ...
        <td style={s.cell} onClick={(e) => e.stopPropagation()}>
          <div style={{ position: "relative" }}>
            <div style={{ ...s.pill, color: getStatus(task.status).color, background: getStatus(task.status).bg, cursor: "pointer" }}
              onClick={(e) => toggleCell("status", e)}>
        ...
        <td style={s.cell} onClick={(e) => e.stopPropagation()}>
          <div style={{ position: "relative" }}>
            <div style={{ ...s.pill, color: getPriority(task.priority).color, background: getPriority(task.priority).bg, cursor: "pointer" }}
              onClick={(e) => toggleCell("priority", e)}>
        ...
        <td style={s.cell} onClick={(e) => e.stopPropagation()}>
          <div style={{ position: "relative" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7, cursor: "pointer" }} onClick={(e) => toggleCell("person", e)}>
              <div style={s.avatar}>{task.person}</div>
              <span style={{ fontSize: 12, color: "#777" }}>{PEOPLE.find((p) => p.id === task.person)?.name ?? task.person}</span>
            </div>
            {openCell === "person" && (
              <CellDropdown onClose={() => setOpenCell(null)}>
                {PEOPLE.map((p) => (
                  <div key={p.id} style={s.cdItem} onClick={() => { onUpdate({ person: p.id }); setOpenCell(null); }}>
        ...
        <td style={s.cell} onClick={(e) => e.stopPropagation()}>
          <div style={{ position: "relative" }}>
            <div style={{ cursor: "pointer" }} onClick={(e) => toggleCell("date", e)}>
              <span style={{ fontSize: 11, color: dateColor(task.due, task.done) }}>
                {formatDate(task.due)}
              </span>
            </div>
            {openCell === "date" && (
        ...
        <>
          ...
          <tr style={{ background: "#191919", borderBottom: "0.5px solid #1e1e1e" }}>
            <td colSpan={5} style={{ paddingLeft: 56, paddingTop: 7, paddingBottom: 7, background: "#191919" }}>
              <div style={s.addSubtaskBtn} onClick={() => onAddSubtask()}>
                + Add subtask
              </div>
            </td>
          </tr>
...
function FilterDropdown({ label, icon, isOpen, isActive, onToggle, children }) {
  return (
    <div style={{ position: "relative" }} onClick={(e) => e.stopPropagation()}>
      <div style={{ ...s.filterBtn, ...(isActive ? s.filterBtnActive : {}) }} onClick={onToggle}>
        {icon}{label}
      </div>
...
// ── Styles ────────────────────────────────────────────────────────────────────
const s = {
  page:            { background: "#141414", height: "100%", minHeight: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" },
  ...
  searchInput:     { background: "transparent", border: "none", outline: "none", fontSize: 12, color: "#888", width: 160 },
  ...
  table:           { width: "100%", borderCollapse: "collapse", minWidth: 700, background: "#141414" },
  ...
  taskRow:         { borderBottom: "0.5px solid #1e1e1e", transition: "opacity 0.15s, background 0.15s", cursor: "grab" },
